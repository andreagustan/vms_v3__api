...
{
    public class T_PengambilanBarangDetailServices : IT_PengambilanBarangDetail
    {
        ...
        public T_PengambilanBarangDetailServices(IRepository _repository, IMapper _mapper, ILogger<Logs> _logger, IHelpers _helpers, IAppsLog _appsLog)
        {
        ...
        public async Task<(bool Status, object Result, string Message)> ListObject(ListPageExt Items)
        {
            try
            {
                ...
                var Rs = await repository.executeProcedure<object>("pT_PengambilanBarangDetail_View", p);
