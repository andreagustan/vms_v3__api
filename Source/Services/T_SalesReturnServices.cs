...
{
    public class T_SalesReturnServices : IT_SalesReturn
    {
        ...
        public T_SalesReturnServices(IRepository _repository, IMapper _mapper, ILogger<Logs> _logger, IHelpers _helpers, IAppsLog _appsLog)
        {
        ...
        public async Task<(bool Status, object Result, string Message)> ListObject(ListPageExt Items)
        {
            try
            {
                ...
                var Rs = await repository.executeProcedure<object>("pT_SalesReturn_View", p);
        ...
        public async Task<(bool Status, object Result, string Message)> DeleteById(CommonDelete Items)
        {
            try
            {
                ...
                var Rs = await repository.executeProcedure<object>("pT_SalesReturn_Del", p);
        ...
        public async Task<(bool Status, object Result, string Message)> BulkUpdate(T_SalesReturn_BulkUpdate Items)
        {
            try
            {
                string NameSp = "";
                ...
                {
                    NameSp = "pT_SalesReturn_Bulk_JSON";

                    p.Add("@formdata", StringHelpers.PrepareJsonstring(Items));
                }
                else
                {
                    NameSp = "pT_SalesReturn_Bulk";
                    ...
                    p.Add("@SRetNo", Items.SRetNo);
                    p.Add("@SRetDate", Items.SRetDate);
                    ...
                    p.Add("@dt", ds.Tables[0].AsTableValuedParameter("tT_SalesReturnDetail"));
                }
        ...
        public DataSet GetDetail()
        {
            ...
            dt.Columns.Add(new DataColumn("SRetNo", typeof(string)));
