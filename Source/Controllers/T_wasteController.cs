...
{
    public partial class T_wasteController : BaseApiController
    {
        ...
        private readonly IT_waste _T_waste;

        public T_wasteController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IT_waste T_waste)
        {
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_Waste_View";
                ...
                var Rs = await _T_waste.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        public async Task<IActionResult> Delete([FromQuery] CommonDelete Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_Waste_Del";
                DtLog.Request = StringHelpers.PrepareJsonstring(Items);
                DtLog.FlagData = ConstValue.LogAdd;
                var RsLog = await _appsLog.WriteAppsLogAsync(DtLog);
                DtLog.Id = !RsLog.Status ? "0" : RsLog.Id;
                ...
                var Rs = await _T_waste.DeleteById(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
        ...
        [Produces("application/json")]
        [HttpPost("BulkUpdate")]
        public async Task<IActionResult> BulkUpdate([FromBody] T_Waste_BulkUpdate Items)
        {
            var DtLog = new LogsDto();
            DtLog.UserId = GetUserId();
            DtLog.Action = Request.Method;
            ...
            try
            {
                ...
                if (Items.JSONProcess) {
                    DtLog.Description = "pT_Waste_Bulk_JSON";
                } else {
                    DtLog.Description = "pT_Waste_Bulk";
                }
                ...
                var Rs = await _T_waste.BulkUpdate(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
