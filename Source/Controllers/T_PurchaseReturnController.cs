...
{
    public partial class T_PurchaseReturnController : BaseApiController
    {
        ...
        private readonly IT_PurchaseReturn _T_PurchaseReturn;

        public T_PurchaseReturnController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IT_PurchaseReturn T_PurchaseReturn)
        {
            ...
            _soapSSOws = soapSSOws;
            _T_PurchaseReturn = T_PurchaseReturn;
        }
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_PurchaseReturn_View";
                ...
                var Rs = await _T_PurchaseReturn.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        public async Task<IActionResult> Delete([FromQuery] CommonDelete Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_PurchaseReturn_Del";
                DtLog.Request = StringHelpers.PrepareJsonstring(Items);
                DtLog.FlagData = ConstValue.LogAdd;
                var RsLog = await _appsLog.WriteAppsLogAsync(DtLog);
                DtLog.Id = !RsLog.Status ? "0" : RsLog.Id;
                ...
                var Rs = await _T_PurchaseReturn.DeleteById(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
        ...
        [Produces("application/json")]
        [HttpPost("BulkUpdate")]
        public async Task<IActionResult> BulkUpdate(T_PurchaseReturn_BulkUpdate Items)
        {
            var DtLog = new LogsDto();
            DtLog.UserId = GetUserId();
            DtLog.Action = Request.Method;
            ...
            try
            {
                ...
                {
                    DtLog.Description = "pT_PurchaseReturn_Bulk_JSON";
                }
                else
                {
                    DtLog.Description = "pT_PurchaseReturn_Bulk";
                }
                ...
                var Rs = await _T_PurchaseReturn.BulkUpdate(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
