...
{
    public partial class T_wasteDetailController : BaseApiController
    {
        ...
        private readonly IT_wasteDetail _T_wasteDetail;

        public T_wasteDetailController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IT_wasteDetail T_wasteDetail)
        {
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_WasteDetail_View";
                ...
                var Rs = await _T_wasteDetail.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        public async Task<IActionResult> Delete([FromQuery] CommonDelete Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_WasteDetail_Del";
                DtLog.Request = StringHelpers.PrepareJsonstring(Items);
                DtLog.FlagData = ConstValue.LogAdd;
                var RsLog = await _appsLog.WriteAppsLogAsync(DtLog);
                DtLog.Id = !RsLog.Status ? "0" : RsLog.Id;
                ...
                var Rs = await _T_wasteDetail.DeleteById(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
