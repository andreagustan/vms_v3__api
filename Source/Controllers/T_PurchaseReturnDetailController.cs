...
{
    public partial class T_PurchaseReturnDetailController : BaseApiController
    {
        ...
        private readonly IT_PurchaseReturnDetail _T_PurchaseReturnDetail;

        public T_PurchaseReturnDetailController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IT_PurchaseReturnDetail T_PurchaseReturnDetail)
        {
            ...
            _soapSSOws = soapSSOws;
            _T_PurchaseReturnDetail = T_PurchaseReturnDetail;
        }
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_PurchaseReturnDetail_View";
                ...
                var Rs = await _T_PurchaseReturnDetail.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        public async Task<IActionResult> Delete([FromQuery] CommonDelete Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_PurchaseReturnDetail_Del";
                DtLog.Request = StringHelpers.PrepareJsonstring(Items);
                DtLog.FlagData = ConstValue.LogAdd;
                var RsLog = await _appsLog.WriteAppsLogAsync(DtLog);
                DtLog.Id = !RsLog.Status ? "0" : RsLog.Id;
                ...
                var Rs = await _T_PurchaseReturnDetail.DeleteById(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
