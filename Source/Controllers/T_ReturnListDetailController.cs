...
{
    public partial class T_ReturnListDetailController : BaseApiController
    {
        ...
        private readonly IT_ReturnListDetail _T_ReturnListDetail;

        public T_ReturnListDetailController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IT_ReturnListDetail T_ReturnListDetail)
        {
            ...
            _soapSSOws = soapSSOws;
            _T_ReturnListDetail = T_ReturnListDetail;
        }
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_ReturnListDetail_View";
                ...
                var Rs = await _T_ReturnListDetail.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        public async Task<IActionResult> Delete([FromQuery] CommonDelete Items)
        {
            ...
            try
            {
                ...
                DtLog.Description = "pT_ReturnListDetail_Del";
                DtLog.Request = StringHelpers.PrepareJsonstring(Items);
                DtLog.FlagData = ConstValue.LogAdd;
                var RsLog = await _appsLog.WriteAppsLogAsync(DtLog);
                DtLog.Id = !RsLog.Status ? "0" : RsLog.Id;
                ...
                var Rs = await _T_ReturnListDetail.DeleteById(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
