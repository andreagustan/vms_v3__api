...
{
    public partial class T_DoController : BaseApiController
    {
        ...
        private readonly IDo _Do;

        public T_DoController(IRepository repository, IMapper mapper, IAppsLog appsLog, IHelpers helpers, ISoapSSO soapSSOws, IDo Do)
        {
        ...
        public async Task<IActionResult> GetListPage([FromQuery] ListPageExt Items)
        {
            ...
            try
            {
                ...
                var Rs = await _Do.ListObjectExt(Items);
                if (Rs.Status)
                {
        ...
        [Produces("application/json")]
        [HttpPost("BulkUpdate")]
        public async Task<IActionResult> BulkUpdate([FromBody] T_DORequest Items)
        {
            var DtLog = new LogsDto();
            DtLog.UserId = GetUserId();
            DtLog.Action = Request.Method;
            ...
            try
            {
                ...
                var Rs = await _Do.BulkUpdate(Items);
                if (Rs.Status)
                {
                    DtLog.Response = StringHelpers.PrepareJsonstring(Rs.Result);
                    _appsLog.WriteAppsLog(DtLog);
